...
class TestFields(SettingsTestCase):
    def setUp(self):
        ...
        class FormSingle(forms.Form):
            obj = CachedModelChoiceField(objects=lambda: self.cached_list, required=False)

        class FormMultiple(forms.Form):
            obj = CachedModelMultipleChoiceField(objects=lambda: self.cached_list, required=False)
    ...
    def test_modelchoicefield_objects_arg(self):
        ...
        as_list_of_tuples = CachedModelChoiceField(objects=lambda: list_of_tuples)
        ...
        self.assertEqual(set(as_list.objects.keys()), set(smart_text(x.pk) for x in self.cached_list))
        ...
        self.assertEqual(choices_without_empty_label, [(smart_text(x.pk), smart_text(x)) for x in self.cached_list])

    def test_modelmultiplechoicefield_objects_arg(self):
        """
        Test, how the field accepts different types of ``objects`` argument.
        """
        as_list = CachedModelMultipleChoiceField(objects=lambda: self.cached_list)
        as_iterable = CachedModelMultipleChoiceField(objects=lambda: iter(self.cached_list))
        list_of_tuples = [(x.pk, x) for x in self.cached_list]
        as_list_of_tuples = CachedModelMultipleChoiceField(objects=lambda: list_of_tuples)
        ...
        # ``choices`` should be a list as ``[(smart_text(pk1), smart_text(obj1)), ...]``
        self.assertEqual(as_list.choices, [(smart_text(x.pk), smart_text(x)) for x in self.cached_list])
