...
from cached_modelforms import ModelForm, CachedModelChoiceField, CachedModelMultipleChoiceField
...
def formfield_callback(f, **kwargs):
    '''
    Sample ``formfield_callback`` function that cacnges the widget for any
    ``CharField`` in model.
    '''
...
class TestForms(SettingsTestCase):
    ...
    def test_modelform_single_without_objects(self):
        ...
        '''
        form = self.ModelFormSingleWithoutObjects({'fk_field': unicode(self.obj1.pk),
                                                   'name': u'Name1'})
    ...
    def test_modelform_multiple_without_objects(self):
        ...
        '''
        form = self.ModelFormMultipleWithoutObjects({'m2m_field': [unicode(self.obj1.pk),
                                                                   unicode(self.obj2.pk)],
                                                     'name': u'Name1'})
